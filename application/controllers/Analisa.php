...
class Analisa extends Kominfo 
{
	...
	public function detail_analisa($param = 0)
	{
		...
		$this->data = array(
			...
			// 'data' => $this->analisa->get_data1($param),
			'konversi' => $this->konversi->gett_all($param),
			'sub_kriteria' => $this->analisa->get_sub_urut($param),
			'get_k' => $this->analisa->get_subkriteria($param),
			//'get_ki' => $this->analisa->get_kriteria($id),
			//'id_kriteria' => $id,
			'id_pelamar' => $param,
			'tampil' => $this->analisa->pengurangan_nilai($param),
			'get_penilaian' => $this->analisa->join($param),
		);
